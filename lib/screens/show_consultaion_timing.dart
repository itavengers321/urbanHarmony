...
import 'package:flutter_project_2208e/screens/mybookings.dart';
import 'package:flutter_project_2208e/widgets/custom_drawer.dart';

class ViewConsultationsUserPage extends StatefulWidget {
  const ViewConsultationsUserPage({super.key});
  static const String routeName = '/ViewConsultationsUserPage';

  @override
  _ViewConsultationsUserPageState createState() =>
      _ViewConsultationsUserPageState();
}

class _ViewConsultationsUserPageState extends State<ViewConsultationsUserPage> {
  ...
  Future<void> _fetchConsultations() async {
    final snapshot = await _consultationRef.get();
  ...
  Future<void> _bookConsultation(String key) async {
    await _consultationRef.child(key).update({'status': 'booked'});
    await _consultationRef.child(key).update({'userUid': uuid});
        ...
        child: consultations.isEmpty
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    color: status == 'available' || status == 'booked'
                        ? Colors.grey
                        ...
                        children: [
                          if (status == 'available')
                            IconButton(
                              ...
                              onPressed: () => _bookConsultation(key),
                            ),
